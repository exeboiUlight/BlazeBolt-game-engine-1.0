...
**BlazeBolt Engine** — это 2D-движок с Lua 5.4 скриптингом, OpenGL 3.3 рендерингом, физикой на Box2D, аудио, сетевыми функциями и процедурной генерацией шумов.

Все функции движка доступны через глобальную таблицу `BlazeBolt`:
...
Некоторые функции зарегистрированы как глобальные (без таблицы `BlazeBolt`):
...
**Тип `Entity`** — это целочисленный идентификатор (integer), возвращаемый функциями создания сущностей. Используйте его для управления созданной сущностью через соответствующие методы.

**Жизненный цикл** движка вызывает Lua-функции `Start()`, `Update(dt)`, `Draw()`, `End()` автоматически.

**Типы данных:** `number` (float), `integer`, `string`, `boolean`, `nil`, `table`, `Entity` (integer), `Vector2`, `Vector3`, `Vector4`, `Matrix3x3`.
...
Создаёт новый спрайт с текстурой.
...
Устанавливает область текстуры (спрайтшит).
...
Устанавливает точку.origin (центр вращения/масштабирования).
...
Устанавливает цвет спрайта (тинт).
...
Пакетная отрисовка множества спрайтов с одной текстурой для повышения производительности.
...
Создаёт анимированный спрайт из спрайтшита.
...
Колесо состояний (AnimationWheel) — это простой способ управлять набором анимаций для одного анимированного спрайта. Каждое состояние — это имя + GIF-файл + настройки воспроизведения. Переключение состояний автоматически загружает нужный GIF и применяет параметры.
...
Меш — это специальный объект с произвольной геометрией, который может иметь кастомный шейдер. Идеально подходит для рендеринга пользовательских фигур, пост-эффектов и визуальных шейдеров.

### CreateMesh

Создаёт пустой 2D меш.
