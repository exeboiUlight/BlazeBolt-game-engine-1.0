...
IMGUI_NODE_EDITOR_API bool HasAnyLinks(PinId pinId);
IMGUI_NODE_EDITOR_API int BreakLinks(NodeId nodeId);
IMGUI_NODE_EDITOR_API int BreakLinks(PinId pinId);
...
IMGUI_NODE_EDITOR_API ImGuiMouseButton GetBackgroundDoubleClickButtonIndex();

IMGUI_NODE_EDITOR_API bool GetLinkPins(LinkId linkId, PinId* startPinId, PinId* endPinId);
    ...
    : SafeType<uintptr_t, Tag>
{
    ...
    SafePointerType()
        : SafeType<uintptr_t, Tag>(0)
    {
    }

    explicit SafePointerType(uintptr_t value)
        : SafeType<uintptr_t, Tag>(value)
    {
    }

    template <typename T = void> explicit SafePointerType(T* ptr): SafeType<uintptr_t, Tag>(reinterpret_cast<uintptr_t>(ptr)) {}
...
template <typename Tag>
const Tag SafePointerType<Tag>::Invalid = Tag(0);
